...
const Grid = ({ titles, tableData, setData, noFilter=false, ignoreFilter=[], loading=false, children }: iGridProps) => {
    ...
    function handleSort(dados: any[]){
        ...
        if(!order) return setData(sortable)
        sortable.sort( (a, b) => {
            var x = a[orderBy]
            var y = b[orderBy]
    ...
    useEffect(() => {
        ...
        const newArr = tableData.slice(0)
        ...
        if(isCombined) {
            ...
            newArr.forEach( item => {
                Object.entries(item).forEach(([, value]) => {
                    if(selectedFilters.includes(value)) filteredArr.push(item)
                })
        })
        }
        setData(filteredArr)
    ...
    useEffect(() => {
        const dados = tableData.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
        handleSort(dados)
    }, [order, orderBy])
                                ...
                                return (
                                    ...
                                    >
                                        <Box sx={{ width: '100%', display: 'flex', justifyContent: 'space-between' }}>
                                            ...
                                            {
                                                (!ignoreFilter.includes(formattedTitle) && !noFilter) &&
...
export const Header = ({ title, children, ...props }) => {
    return (
        <Box component="header" className="flex w-full justify-between flex-col mb-4 px-5 sm:px-0" {...props}>
